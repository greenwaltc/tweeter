...
public class StatusService extends Service{
    ...
    public SimpleResponse postStatus(PostStatusRequest request) {
        ...
        daoFactory.getStoryDAO().insert(request.getStatus());

        boolean hasMorePages = true;
        String lastReceiverAlias = null;
        do {
            // Get the aliases of users following the sender
            UsersRequest usersRequest = new UsersRequest(request.getAuthToken(), senderAlias, 10, lastReceiverAlias);
            ...
            // For each follower alias
            for (User receiver : batchReceivers) {
                daoFactory.getFeedDAO().insert(receiver.getAlias(), request.getStatus());
            }
    ...
    public StatusesResponse getFeed(StatusesRequest request) {
        ...
        Pair<List<Status>, Boolean> getFeedResult = daoFactory.getFeedDAO().getFeed(request);
    ...
    public StatusesResponse getStory(StatusesRequest request) {
        ...
        Pair<List<Status>, Boolean> getStoryResult = daoFactory.getStoryDAO().getStory(request);
